...
public final class VPTrades {
    ...
    private static VillagerTrades.ItemListing parseTrade(JsonObject trade, String profession) {
        ...
        switch (type) {
            ...
            case "villagersplus:sell_potion" -> {
                ...
                final ItemStack potionCostA = costA;
                final ItemStack potionCostB = costB;
                final ItemStack bottle = result;
                return (trader, random) -> new MerchantOffer(
                        potionCostA.copy(), potionCostB.copy(), randomPotion(bottle, random),
        ...
        final ItemStack finalCostA = costA;
        final ItemStack finalCostB = costB;
        final ItemStack finalResult = result;
        return (trader, random) -> new MerchantOffer(
                finalCostA.copy(), finalCostB.copy(), finalResult.copy(), maxUses, xp, multiplier);
    }
    ...
    private static ItemStack randomPotion(ItemStack bottle, net.minecraft.util.RandomSource random) {
        ...
        Potion potion = BuiltInRegistries.POTION.getRandom(random).map(Holder::value).orElse(null);
        if (potion != null) {
